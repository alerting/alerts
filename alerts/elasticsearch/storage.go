...
import (
	...
	"github.com/alerting/alerts/alerts"
...
func (s *Storage) Find(ctx context.Context, criteria *alerts.FindCriteria) (*alerts.FindResult, error) {
	...
	if criteria.AreaDescription != "" {
		log.WithField("value", criteria.Instruction).Debug("area.description")
		span.SetTag("area.description", criteria.AreaDescription)
		query = query.Must(elastic.NewQueryStringQuery(criteria.AreaDescription).Field("area.description"))
	}
	...
	if criteria.Point != nil {
		...
		areaQuery = areaQuery.Must(elastic.NewBoolQuery().
			Should(NewGeoShapeQuery("area.polygons").SetPoint(criteria.Point.Lat, criteria.Point.Lon)).
			Should(NewGeoShapeQuery("area.circles").SetPoint(criteria.Point.Lat, criteria.Point.Lon)))
	}
